...
export default function Headbar({ onMenuClick }: HeadbarProps) {
  ...
  const handleLogout = async () => {
    ...
    try {
      // Hapus cookie session di server (ini yang dicek middleware)
      await fetch('/api/auth/logout', { method: 'POST' })
    } catch {
      // ignore, tetep lanjut logout di client
    }
  ...
  return (
    <>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl shadow-lg overflow-hidden">
                  ...
                  {/* Menu Ganti Password - HANYA UNTUK ADMIN */}
                  {user?.role === 'admin' && (
                    <>
                      <button
                        onClick={() => {
                        ...
                        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium text-on-surface hover:bg-surface-container/50 transition-colors"
                      >
      ...
      {/* Modal Ganti Password - tetap dirender, tapi hanya bisa dipicu oleh admin */}
      {showChangePassword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-surface-container-lowest rounded-2xl shadow-xl w-full max-w-md mx-4 p-6 border border-outline-variant">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-surface-container rounded-lg border border-outline-variant focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary text-on-surface"
                    required
                  />
                </div>
                <div>
